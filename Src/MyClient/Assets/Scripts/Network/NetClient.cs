    ...
    class NetClient : MonoSingleton<NetClient>
    {
        ...
        void DoConnect()
        {
            ...
            try
            {
                ...
                this.clientSocket.Blocking = true;//请求连接服务器时阻塞，等待到链接结果
                ...
                bool success = result.AsyncWaitHandle./*等待事件完成*/WaitOne(NetConnectTimeout);
                if (success)
                {
            ...
            if (this.clientSocket.Connected)//再次校验
            {
        ...
        bool ProcessRecv()
        {
            ...
            try
            {
                ...
                //类Linux服务器
                ret = this.clientSocket.Poll(0, SelectMode.SelectRead);//同步选择读取 学名TODO
                if (ret)
                {
